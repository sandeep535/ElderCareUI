...
import { fetchDashboardStats, fetchDashboardTaskCount } from '../../api/endpoints'
...
export default function StatsSection() {
  const { data: stats,      loading: statsLoading,  error: statsError,  execute: refetchStats }  = useApi(fetchDashboardStats)
  const { data: taskCounts, loading: tasksLoading,  error: tasksError,  execute: refetchTasks }  = useApi(fetchDashboardTaskCount, [TODAY])
  ...
  if (error) {
    return <ErrorState message={error} onRetry={() => { refetchStats(); refetchTasks(TODAY) }} />
  }
  ...
  const STAT_CARDS = [
    ...
    },
    {
      ...
      change:      taskCounts ? `${taskCounts.total} total today` : 'Due today',
      type:        'info',
      changeClass: '',
      icon: (
        <svg viewBox="0 0 24 24" fill="none">
    ...
    },
    {
      ...
      change:      taskCounts ? `${taskCounts.total} total today` : 'Today',
      type:        'success',
      changeClass: 'positive',
      icon: (
        <svg viewBox="0 0 24 24" fill="none">
